...
class Net(nn.Module):
    ...
    def forward(self, x_depth):
        ...
        concat_feat = out_depth#torch.cat((out_rgb, out_depth), dim=1) # out:[batch_size,2048,7,7]
...
class Dqn():
    ...
    def store_trans(self, state_path, action, reward, next_state_path,done):
        ...
        next_state_depth = my_utils.copy_depth_to_3_channel(next_state_d).reshape(1, 3, DIM_STATES[0], DIM_STATES[1])#origin depth => heightmap, copy to 3 channels
        ...
        target_singleChannel_q_map = self.eval_net.forward(state_depth)#dim:[1,1,224,224],CHANNEL=1
        # x,y,c = my_utils.translate_actionID_to_XY_and_channel(action)
        old_val = target_singleChannel_q_map[0][0][x][y]
        # old_val = target[0][action]
        target_val_singleChannel_q_map = self.target_net.forward(next_state_depth)#dim:[1,1,224,224]
    ...
    def choose_action(self, state_path,EPSILON):
        ...
        if selected_ac_type == 0:#origin predicted action
            target_multiChannel_q_map = self.eval_net.forward(state_depth)  # dim:[INPUT_IMAGES,1,224,224]
    ...
    def load_batch_data(self,batch_list):#batch_list.dim:[batch_size]
        ...
        return torch.cuda.FloatTensor(batch_state_depth), torch.cuda.LongTensor(batch_action), torch.cuda.FloatTensor(batch_reward), torch.cuda.FloatTensor(batch_next_state_depth)
    ...
    def learn(self):
        ...
        batch_state_depth,batch_action,batch_reward,batch_next_state_depth = self.load_batch_data(mini_batch)#dim:[1]

        eval_singleChannel_q_map = self.eval_net(batch_state_depth)  # dim:[BATCH_SIZE,1,224,224]
...
def main():
    ...
    for episode in range(Train_Configs.EPISODES):
        ...
        while True:
            ...
            if net.memory.tree.n_entries >= 1000:
                l, mean_q = net.learn()
            ...
            if success_grasp_obj_num == obj_num or step_count_every_ep == 50:
