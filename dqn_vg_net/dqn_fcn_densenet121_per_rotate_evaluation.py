...
class Dqn():
    ...
    def store_trans(self, state_path, action, reward, next_state_path,done):
        ...
        # normlize
        state_depth = (state_depth - np.min(state_depth)) / (np.max(state_depth) - np.min(state_depth))
        next_state_depth = (next_state_depth - np.min(next_state_depth)) / (np.max(next_state_depth) - np.min(next_state_depth))
        # numpy to tensor
    ...
    def load_batch_data(self,batch_list):#batch_list.dim:[batch_size]
        ...
        # normlize
        batch_state_depth = (batch_state_depth - np.min(batch_state_depth)) / (np.max(batch_state_depth) - np.min(batch_state_depth))
        batch_next_state_depth = (batch_next_state_depth - np.min(batch_next_state_depth)) / (np.max(batch_next_state_depth) - np.min(batch_next_state_depth))
...
def main():
    ...
    for episode in range(Train_Configs.EPISODES):
        ...
        while True:
            ...
            if done == 1 or step_count_every_ep == 50:
...
def evaluation(model_path):
    ...
    log_txt_result = open('model_eval_log.txt','a')
    ...
    for episode in range(100000):
        ...
        if done != -1:
            valid_ep_count += 1
            grasp_suc_rate.append(success_grasp_obj_count / step_count)
        ...
        if valid_ep_count == 50:
            break

    log_txt_result.write(model_path + ': ' + str(completion_count / 50) + ',' + str(np.mean(np.array(grasp_suc_rate))) + '\n')
    log_txt_result.close()

if __name__ == '__main__':
    #-------------------------------------------------------------------
    #  evaluate the performance every 1000 step to find the best policy
    #-------------------------------------------------------------------

    start_step = 10000
    end_step =220000
    for step_id in range(start_step,end_step,2000):
        print('evaluate step ',str(step_id),'.............')
        evaluation('step_'+str(step_id)+'_params.pkl')
